...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _deploy_server(self):
        ...
        if not p.exists():
            ...
            return False
        cp = self.adb.push(p, "/sdcard/Download/Just_server_arm64")
        self._log(f"adb push: rc={cp.returncode}")
        if cp.returncode != 0:
            return False
        cp_install = self.adb.shell(
            "su -c 'rm -f /data/local/tmp/Just_server_arm64; cp /sdcard/Download/Just_server_arm64 /data/local/tmp/Just_server_arm64 && chmod 777 /data/local/tmp/Just_server_arm64'",
            timeout=30,
        )
        self._log(f"install server to tmp: rc={cp_install.returncode}")
        return cp_install.returncode == 0
    ...
    def _ensure_server_ready_on_startup(self):
        ...
        if not self._deploy_server():
            self._styled_alert("环境检查失败", "推送服务端到 /sdcard/Download 或安装到 /data/local/tmp 失败。")
            return False
